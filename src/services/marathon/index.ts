...
import { Product, SpCollection, SpDrawerTypes, SpFinish } from '../../typings/Product';
import { URL } from 'url';
...
export const marathonService = ({ db }: MarathonServiceDependencies) => {
  ...
  const syncProduct = async () => {
    ...
    if (products && products.length > 0) {
      ...
      const partNumbers = products.map(({ node: product }) => product.partNumber?.trim() as string).filter((x) => !!x);
